...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-6">
            ...
            <h3>Invoice ID: {{$invoice->id ?? 'Unknown ID'}}</h3>
            <h3>Invoice Date: {{\Carbon\Carbon::parse($invoice->created_at)->format('M d Y')}}</h3>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-6">
            ...
            <hr>
            <div class="row">
                Customer: {{$invoice->customer->firstName ?? 'Unknown Customer'}} {{$invoice->customer->lastName ?? ''}} - @if($invoice->customer->phoneNumber) {{ '('.substr($invoice->customer->phoneNumber, 0, 3).') '.substr($invoice->customer->phoneNumber, 3, 3).'-'.substr($invoice->customer->phoneNumber,6)  }} @endif <br />
                Address: {{$invoice->customer->address ?? ''}} <br/>
                Date of Service: @if($invoice->dateofService){{\Carbon\Carbon::parse($invoice->dateofService)->format('M d Y')}} @else TBD @endif <br />
                Detail Service Type: @if($invoice->estimate->detilType == 1) Shop Detail @elseif($invoice->detailType == 2) Mobile Detail @else T.B.D @endif <br />
                Arrival Window: @if($invoice->estimate->arrivalTime){{\Carbon\Carbon::parse($invoice->estimate->arrivalTime)->format('H:i')}} to {{\Carbon\Carbon::parse($invoice->estimate->arrivalTime)->addHours(3)->format('H:i')}}@else TBD @endif <br/>
            </div>
        </div>
        <div class="col-6">
            ...
            @if($invoice->estimate->vehicle)
                VIN Number: {{$invoice->estimate->vehicle->vehicleInfo->vin ?? 'VIN not provided at time of estimate.'}} <br />

                Vehicle Year: {{$invoice->estimate->vehicle->vehicleInfo->year ?? ''}} <br />


                Vehicle Make: {{$invoice->estimate->vehicle->vehicleInfo->make ?? ''}} <br />


                Vehicle Model/Style: {{$invoice->estimate->vehicle->vehicleInfo->model ?? ''}} - {{$invoice->estimate->vehicle->vehicleInfo->style ?? ''}} <br />


                Vehicle Color: {{$invoice->estimate->vehicle->vehicleInfo->colorInfo->description ?? ''}}
            @else
    ...
    <div class="row">
        <table class="table table-striped">
            ...
            <tr>
                ...
                <td>{{$invoice->estimate->listPrice ?? ''}}</td>
                <td>{{$invoice->total ?? ''}}</td>
                <td @if($invoice->totalPaid >= $invoice->deposit) bg-success @endif >{{$invoice->deposit ?? ''}}</td>
            </tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="row mt-5">
        <div class="col-12">
            <small>
                ...
                Based upon our initial estimate, if the prices change or additional parts and labor are required, we will inform you prior to
